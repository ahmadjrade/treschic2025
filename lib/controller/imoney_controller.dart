...
import 'package:fixnshop_admin/model/imoney_model.dart';
...
class ImoneyController extends GetxController {
  RxList<ImoneyModel> imoney = <ImoneyModel>[].obs;
  RxList<ImoneyModel> displayedIMoney = <ImoneyModel>[].obs; // Displayed imoney
  bool isDataFetched = false;
  String result = '';
  RxBool isLoading = false.obs;
  RxBool isFetchingMore = false.obs; // To track loading more data
  Rx<ImoneyModel?> SelectedIMoney = Rx<ImoneyModel?>(null);
  ...
  void clearSelectedCat() {
    SelectedIMoney.value = null;
    imoney.clear();
    displayedIMoney.clear(); // Clear displayed imoney as well
  }
  void onClose(){ 
...
//   List<ImoneyModel> SearchExpenses(String query) {
...
//         .toList();
//   }
//   List<ImoneyModel> SearchExpensesYday(String query) {
...
//       .toList();
// }
//   List<ImoneyModel> searchExpensesAll(String query) {
//     String dateString = dateController.getFormattedDate();
//     List<String> dateParts = dateString.split('-');
//     String month = dateParts[1].length == 1 ? '0${dateParts[1]}' : dateParts[1];
//     String day = dateParts[2].length == 1 ? '0${dateParts[2]}' : dateParts[2];
...
// List<ImoneyModel> searchExpensesMonth(String query) {
...
//   }
  ...
  // List<ImoneyModel> SearchDueInvoices(String query) {
  //   String dateString = dateController.getFormattedDate();
  //   List<String> dateParts = dateString.split('-');
  //   String month = dateParts[1].length == 1 ? '0${dateParts[1]}' : dateParts[1];
  //   String day = dateParts[2].length == 1 ? '0${dateParts[2]}' : dateParts[2];
  ...
  RxDouble total_yday = 0.0.obs;
   ...
   RxDouble total_all = 0.0.obs;
    ...
    RxDouble dollar_money = 0.0.obs;
      RxDouble lebanese_money = 0.0.obs;
    ...
    formattedTime = dateController.getFormattedTime();
    Username = sharedPreferencesController.username;
    dollar_money.value = 0;
      lebanese_money.value = 0;


    List<ImoneyModel> totalofinvoices = imoney
        .where((expense) =>
            expense.Username == Username.value &&
            expense.date.contains(formattedDate) )
        .toList();
    for (int i = 0; i < totalofinvoices.length; i++) {
      dollar_money.value += totalofinvoices[i].dollar_money;  
          lebanese_money.value += totalofinvoices[i].lebanese_money;
...
//   }
  void fetch_imoney() async {
    ...
    if (!isDataFetched) {
      try {
        ...
        final response =
            await http.get(Uri.parse('$domain' 'fetch_initial_money.php'));

        final jsonData = json.decode(response.body);
        if (jsonData is List) {
          final List<dynamic> data = jsonData;
          imoney.assignAll(
              data.map((item) => ImoneyModel.fromJson(item)).toList());
          isDataFetched = true;
          // Initially display the first batch of imoney
          displayedIMoney.assignAll(imoney.take(itemsPerPage));
  ...
  Future<void> loadMoreInvoices() async {
    if (!isFetchingMore.value && displayedIMoney.length < imoney.length) {
      isFetchingMore.value = true;
      int startIndex = currentPage * itemsPerPage;
      int endIndex = startIndex + itemsPerPage;
      if (endIndex > imoney.length) {
        endIndex = imoney.length;
      }

      // Simulate a delay for loading more data
      await Future.delayed(Duration(seconds: 2));

      displayedIMoney.addAll(imoney.sublist(startIndex, endIndex));
      currentPage++;
  ...
  String result2 = '';
   Future<void> InsertInitialMoney(String Dollar_amm, String lebanese_amm) async {
    try {
      Username = sharedPreferencesController.username;
      ...
      String domain = domainModel.domain;
      String uri = '$domain' + 'insert_initial_money.php';

      var res = await http.post(Uri.parse(uri), body: {
        "Dollar_amm": Dollar_amm,
        "lebanese_amm": lebanese_amm,
