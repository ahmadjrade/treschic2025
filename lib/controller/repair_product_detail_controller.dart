...
class RepairProductDetailController extends GetxController {
  RxList<RepairProductDetailModel> repair_product_detail =
      <RepairProductDetailModel>[].obs;
  bool isDataFetched = false;
  String result = '';
  RxBool isLoading = false.obs;
  Rx<RepairProductDetailModel?> SelectedRepairProduct =
      Rx<RepairProductDetailModel?>(null);

  void clearSelectedCat() {
    SelectedRepairProduct.value = null;
    repair_product_detail.clear();
  }
...
//  List<RepairProductDetailModel> searchProducts(String query) {
//     return repair_product_detail.where((product) =>
  ...
  void fetchproductdetails() async {
    if (!isDataFetched) {
      try {
        ...
        final response =
            await http.get(Uri.parse('$domain' + 'fetch_repair_pdetail.php'));

        final jsonData = json.decode(response.body);
        if (jsonData is List) {
          final List<dynamic> data = jsonData;
          //  final List<dynamic> data = json.decode(response.body);
          repair_product_detail.assignAll(data
              .map((item) => RepairProductDetailModel.fromJson(item))
              .toList());
          //category = data.map((item) => Product_Details.fromJson(item)).toList();
          isDataFetched = true;
          isLoading.value = false;

          if (repair_product_detail.isEmpty) {
            print(0);
          } else {
            isDataFetched = true;
            result == 'success';
